...
<div class="tools_bar clearfix">
    ...
    <a  class="icon-remove-2" href="javascript:;" onclick="tools_submit({action:'<?php echo urldecode(Url::urlFormat("/customer/grade_del"));?>',msg:'删除后无法恢复，你真的删除吗？'})" title="删除"> 删除</a>
    <a href='<?php echo urldecode(Url::urlFormat("/customer/grade_edit"));?>' class="icon-plus" > 添加</a>
</div>
<table class="default" >
    <tr>
        ...
        <th >等级名</th>
        <th style="width:170px">需要满足的消费份额</th>
        
    </tr>
    <?php $item=null; $obj = new Query("grade");$obj->page = "1 desc";$items = $obj->find(); foreach($items as $key => $item){?>
        <tr><td style="width:30px"><input type="checkbox" name="id[]" value="<?php echo isset($item['id'])?$item['id']:"";?>"></td>
        <td style="width:70px" class="btn_min"><div class="operat hidden"><a  class="icon-cog action" href="javascript:;"> 处理</a><div class="menu_select"><ul>
                        <li><a class="icon-pencil" href="<?php echo urldecode(Url::urlFormat("/customer/grade_edit/id/$item[id]"));?>"> 编辑</a></li>
                        <li><a class="icon-remove-2" href="javascript:confirm_action('<?php echo urldecode(Url::urlFormat("/customer/grade_del/id/$item[id]"));?>')"> 删除</a></li>
                    </ul></div></div> </td>
        <td><?php echo isset($item['name'])?$item['name']:"";?></td><td style="width:170px"><?php echo isset($item['money'])?$item['money']:"";?></td></tr>
    <?php }?>
</table>
</form>
<div class="page_nav">
