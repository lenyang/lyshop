...
<div class="tools_bar clearfix">
    ...
    <a  class="icon-remove-2" href="javascript:;" onclick="tools_submit({action:'<?php echo urldecode(Url::urlFormat("/distribution/rebate_del"));?>',msg:'删除后无法恢复，你真的删除吗？'})" title="删除"> 删除</a>
    <a class="icon-delicious" href="<?php echo urldecode(Url::urlFormat("/distribution/rebate_list"));?>"> 全部记录</a>
...
<table class="default" >
    ...
    <?php $item=null; $obj = new Query("rebate as rb");$obj->fields = "rb.*,us.real_name as uname,dp.name as pname";$obj->join = "left join agent as us on rb.user_id = us.id  left join dispro as dp on rb.dispro_id = dp.id";$obj->where = "$where";$obj->page = "1 desc";$items = $obj->find(); foreach($items as $key => $item){?>
        <tr><td style="width:30px"><input type="checkbox" name="id[]" value="<?php echo isset($item['id'])?$item['id']:"";?>"></td>
        <td style="width:70px" class="btn_min"><div class="operat hidden"><a  class="icon-cog action" href="javascript:;"> 处理</a><div class="menu_select"><ul>
                <?php if($item['status']==0){?>
                <li><a class="icon-pencil" href="javascript:withdrawAction(<?php echo isset($item['id'])?$item['id']:"";?>);">发放</a></li>
                <?php }?>
        ...
        <td style="width:100px"><?php echo isset($item['rebate_no'])?$item['rebate_no']:"";?></td><td style="width:80px"><?php echo isset($item['uname'])?$item['uname']:"";?></td><td style="width:100px"><?php echo isset($item['pname'])?$item['pname']:"";?></td><td style="width:100px"><?php echo isset($item['lower'])?$item['lower']:"";?></td><td style="width:100px"><?php echo isset($item['disorder_no'])?$item['disorder_no']:"";?></td><td style="width:80px"><?php echo isset($item['qty'])?$item['qty']:"";?></td><td ><?php echo isset($item['money'])?$item['money']:"";?></td><td style="width:60px"><?php if($item['status']==1){?><b class="green">成功</b><?php }else{?><b class="red">等待</b><?php }?></td><td style="width:100px"><?php echo isset($item['create_time'])?$item['create_time']:"";?></td>
        </tr>
    <?php }?>
</table>
</form>
...
<script type="text/javascript">
    ...
    $("#condition").on("click",function(){
        $("body").Condition({input:"#condition_input",okVal:'高级搜索',callback:function(data){tools_submit({action:'<?php echo urldecode(Url::urlFormat("/customer/withdraw_list"));?>',method:'get'});},data:{'us.real_name':{name:'用户名'},'dp.name':{name:'商品名'},'rb.status':{name:'状态',values:{0:'等待',1:'成功'}}
        }});
    });
    function withdrawAction(id)
    {
        art.dialog.open("<?php echo urldecode(Url::urlFormat("/distribution/rebate_audit/id/"));?>"+id,{id:'withdraw-dialog',title:"审批",width:620});
    }
